...
int remove(int u, int x) {
  ...
  if (ls[u] == 0 && rs[u] == 0) return 0;
  if (ls[u] == 0) return rs[u];
  if (rs[u] == 0) return ls[u];
...
int get_rank(int u, int x) {
  if (u == 0) return 0;
  if (key[u] < x) return sz[ls[u]] + cnt[u] + get_rank(rs[u], x);
...
int get_key(int u, int rk) {
  if (sz[ls[u]] + cnt[u] <= rk) return get_key(rs[u], rk - sz[ls[u]] - cnt[u]);
  if (sz[ls[u]] > rk) return get_key(ls[u], rk);
  return key[u];
}
/**
 * @brief 获取节点前驱
...
int get_pre(int u, int x) {
  if (u == 0) return -1e9;
  if (key[u] < x) return max(get_pre(rs[u], x), key[u]);
...
int get_post(int u, int x) {
  if (u == 0) return 1e9;
  if (key[u] > x) return min(get_post(ls[u], x), key[u]);
