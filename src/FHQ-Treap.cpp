...
void upd(int p) { siz[p] = siz[ls[p]] + siz[rs[p]] + 1; }
...
int get_key(int p, int x) {
  ...
  if (siz[ls[p]] + 1 < x) return get_key(rs[p], x - siz[ls[p]] - 1);
  return key[p];
}
int get_pre(int p, int x) {
  if (!p) return INT_MIN;
  if (key[p] >= x) return get_pre(ls[p], x);
  return max(key[p], get_pre(rs[p], x));
}
int get_post(int p, int x) {
  if (!p) return INT_MAX;
  if (key[p] <= x) return get_post(rs[p], x);
